...
def inserir_venda(quantidade: int, valor: float, data: str):
...
"""
def selecionar_venda_data(data: str) -> pd.DataFrame:
    sql_query = """
                SELECT * FROM Venda 
    ...
    try:
        with conectar() as conn:
            ...
            df = pd.read_sql_query(sql_query, conn, params=(data_mes_str, data_ano_str))
            return df
    except Error as e:
...
"""
def selecionar_venda_lucro_total() -> float:
    sql_query = """
